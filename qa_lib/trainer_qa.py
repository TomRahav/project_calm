    ...
    def evaluate(
        ...
        **gen_kwargs,
        ...
        if self.model.decoder.use_shallow_deep:
            total, deep = (
                self.model.decoder.block_op[0],
                self.model.decoder.block_op[self.model.decoder.shallow_exit_layer],
            )
            shallow = total - deep

            # self.model.rollback_num: we should consider redundant operations due to rollback
            block_op_metric = {
                "{}_block_avg".format(metric_key_prefix): (
                    deep * len(self.model.decoder.block_op)
                    + (shallow + self.model.rollback_num)
            ...
            block_op_metric["{}_block_num".format(metric_key_prefix)] = str(
                [
                    shallow,
                    deep,
                    self.model.decoder.parallel_tokens_shallow,
                    self.model.decoder.parallel_tokens_deep,
                    self.model.rollback_num,
                ]
            )
        else:
            block_op_metric = {
                "{}_block_avg".format(metric_key_prefix): sum(
                    self.model.decoder.block_op
                )
